...
- ~~**Restoring the browser's `noCrit` rule made crit builds harder** (SG-148)~~
  — **ANSWERED BY YOU AND REVERTED (SG-159).** All six secondary sources crit
  again; the rule and the browser reasoning behind it are out of the code. What
  it gave back, measured the same way, is in `docs/BOARD.md` SG-159.
