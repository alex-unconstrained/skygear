...
const TUNING = {
  ...
  close: {
    ...
    pressureGrace: 1.2,
    /* v12, after the first v11 playthrough: twelve waves, 4:43, 338 kills,
       played entirely in melee, and the captain was never in danger. Nothing
       was individually broken — the vent gave 15hp on a 1.1s floor, salvage
       dropped on 16% of close kills, SCRAPPER'S LUCK added another 15% at 15hp,
       and FIELD DRESSING paid 1.5/s the whole run because permanent melee means
       permanent pressure. Together that is over a thousand points of healing
       across a run against a 100hp captain, and no single number looked wrong.
       So the ceiling is on the SUM, and the supply comes down as well. */
    vent: { heal: 10, dmg: 40, radius: 200, knock: 280, cd: 2.0 },
    scrapChance: 0.10,         // a close kill drops salvage this often, no card
    scrapHeal: 6,
    dashRefund: 0.30,          // seconds off the dash cooldown per close kill
    healCapPerSec: 12,         // ceiling on ALL healing, in hp/s
    lifestealCapPerSec: 9,     // and a tighter one on healing from damage alone
  },
  ...
  /* v12 — the draft. `rerolls` is run-wide rather than per draft, so spending
     one is a decision about which hand was bad enough to be worth it. */
  draft: { rerolls: 2 },
...
const SHAPES = {
  ...
  /* Passives. They take a slot like anything else and cross with all four
     elements, so they are part of the matrix rather than a category beside it —
     but they never read input. Four actives means four things to press, and a
     build that wants power without more buttons had no way to ask for it. */
  AURA: {
    key:'AURA', noun:'Field', kind:'aura', passive:true,
    desc:'a field around you, always on',
    // A field hits everything inside it continuously, so its damage scales with
    // the crowd — in a horde game that is the strongest scaling in the set, and
    // it asks nothing of the player. Benchmarked at 96 dps against four
    // clustered dummies where the best active managed 62; tuned to sit under a
    // well-played active, which is the deal a passive should offer.
    radius: 150, dmg: 4, tickRate: 1.8, cd: 0, knock: 0,
  },
  PULSE: {
    key:'PULSE', noun:'Pulse', kind:'pulse', passive:true,
    desc:'detonates around you on its own',
    radius: 210, dmg: 34, cd: 4.4, knock: 220,
  },
  SENTRY: {
    key:'SENTRY', noun:'Sentry', kind:'sentry', passive:true,
    desc:'plants a gun that fires for you',
    dmg: 14, cd: 9.0, life: 7.0, range: 420, shotCd: 0.7, knock: 60,
  },
...
/* What a card actually touches, for the draft to show as icons (v12).

   `skill` cards fill a slot and `slot`-targeted cards name theirs, so both are
   already self-describing. The rest were not: an element card silently hit
   every skill of that colour and a captain card hit none of them, and the card
   face said the same thing either way — which is the "is this a new skill or an
   upgrade to one I have" confusion the draft screen was reported for.

   Declared here rather than guessed in the renderer, because the renderer
   guessing is how it ends up wrong the first time a card is added. */
const CARD_SCOPE = {
  // targets ONE skill in a slot
  aoe:'skill', range:'skill', cd:'skill', dmg:'skill', jump:'skill',
  pierce:'skill', widecone:'skill', knockskill:'skill', twin:'skill',
  reelem:'skill', reshape:'skill',
  // touches every skill of one element
  burnDmg:'element', burnDur:'element', slowAmt:'element', brittle:'element',
  stun:'element', knock:'element', scald:'element', elemdmg:'element',
  elemcd:'element',
  // the captain herself
  hp:'captain', spd:'captain', dashcd:'captain', dashdmg:'captain',
  dashchg:'captain', crit:'captain', critx:'captain', scrap:'captain',
  lifesteal:'captain', ventheal:'captain', ventdmg:'captain',
  pressrate:'captain', dressing:'captain',
  // the objective
  boilerhp:'ship', boilerdr:'ship',
  // the deck's ordnance
  kegs:'deck',
  // everything you hold, whatever it is
  fifth:'all', residue:'all', autofire:'all', killboom:'all',
  // arms an empty slot with a new weapon. `skillpick` is not a CARDS entry —
  // skill cards are generated by rollSkillCards and carry `skill`, which
  // cardScope() detects directly.
  newskill:'new',
  // the draft itself
  spares:'meta',
};
...
const CARDS = [
  ...
  { id:'aoe', rarity:'common', weight:S => 10,
    ...
    make:S => { const i = pickSlotByUse(slotsWhere(S, sk => sk.shape !== 'LINE_BURST' && sk.shape !== 'CHAIN'));
  ...
  { id:'range', rarity:'common', weight:S => 10,
    ...
    make:S => { const i = pickSlotByUse(filledSlots(S));
  ...
  { id:'cd', rarity:'common', weight:S => 12,
    ...
    make:S => { const i = pickSlotByUse(slotsWhere(S, sk => sk.mods.cd > 0.42));
  ...
  { id:'jump', rarity:'rare', weight:S => 14,
    ...
    make:S => { const i = pickSlotByUse(slotsWhere(S, sk => sk.shape === 'CHAIN'));
  ...
  { id:'pierce', rarity:'rare', weight:S => 14,
    ...
    make:S => { const i = pickSlotByUse(slotsWhere(S, sk => sk.shape === 'LINE_BURST'));
  ...
  { id:'widecone', rarity:'rare', weight:S => 14,
    ...
    make:S => { const i = pickSlotByUse(slotsWhere(S, sk => sk.shape === 'CONE' && !sk.mods.wideCone));
  ...
  { id:'twin', rarity:'epic', weight:S => 9,
    ...
    make:S => { const i = pickSlotByUse(slotsWhere(S, sk => sk.mods.multi < 2 && sk.shape !== 'RAY'));
  ...
  { id:'elemdmg', rarity:'rare', weight:S => 9 * (slotsWhere(S, sk => true).length > 1 ? 2 : 1),
    ...
    make:S => { const e = pick(ELEMENT_KEYS.filter(k => hasElement(S, k)));
      ...
      return { element: e, title: ELEMENTS[e].name.toUpperCase() + ' CONVERGENCE',
  ...
  { id:'elemcd', rarity:'rare', weight:S => 7 * (slotsWhere(S, sk => true).length > 1 ? 2 : 1),
    can:S => ELEMENT_KEYS.some(e => hasElement(S, e)),
    make:S => { const e = pick(ELEMENT_KEYS.filter(k => hasElement(S, k)));
      return { element: e, title: ELEMENTS[e].name.toUpperCase() + ' CADENCE',
  ...
  // v12: once only. Taken three times it meant 45% of every kill dropping 15hp,
  // on top of the close-kill salvage the deck already gives for free.
  { id:'scrap', rarity:'common', weight:S => 9, can:S => S.mods.scrapChance < 0.15,
    make:S => ({ title:"SCRAPPER'S LUCK", text:'Kills have a 15% chance to drop 12 health.',
  ...
  { id:'dressing', rarity:'common', weight:S => V11 ? 9 : 0, can:S => V11 && S.mods.dressing < 4,
    // v12: below 60% only. Paying out at full health made it a permanent
    // trickle that removed the fail state rather than a comeback.
    make:S => ({ title:'FIELD DRESSING', text:'Below 60% health, heal 2/s while your pressure is above half.',
                 apply:S => { S.mods.dressing += 2; } }) },
  { id:'spares', rarity:'common', weight:S => 7, can:S => S.rerolls < 6,
    make:S => ({ title:'SPARE PARTS', text:'+2 rerolls, for any future draft.',
                 scope:'meta', apply:S => { S.rerolls += 2; } }) },
...
const S = {
  ...
  hintT: 0,
  intro: 0,
  rerolls: 0,               // v12 — spare draws, run-wide
  tel: null,               // v12 — what the player actually did (Telemetry)
  src: null,               // the slot currently resolving, for attribution
  bossRef: null,
  seenTypes: null,
};
...
function resetGame(){
  ...
  S.player = {
    ...
    stealBudget: TUNING.close.lifestealCapPerSec,
    healBudget: TUNING.close.healCapPerSec, crowd: 0,
  };
  resetProps();
  ...
  S.tel = Telemetry.fresh();
  S.src = null;
  S.rerolls = TUNING.draft.rerolls;
}
...
/* Transient visual effects, with a ceiling (v12).

   `fx()` used to push without limit, and measurement is why it now does not: a
   saturated wave-11 frame — four skills firing, a keg chain going off, kills
   landing — held 225 live effects and asked the canvas for 15,968 calls, of
   which 9,354 came from drawing those effects. That is the "it lags when a lot
   is going on" report, and it was never the browser: one ground effect costs
   about forty canvas calls and nothing was counting them.

   The cap scales with the player's own VFX setting, and eviction is by kind:
   the oldest purely decorative effect goes first. `delayPop` (the Colossus death
   sequence) and `stun` (which holds a reference to a live enemy) are never
   evicted, because both mean something. */
const FX_CAP = 76;
const FX_EVICTABLE = { arc:1, cone:1, line:1, aoe:1, pop:1, muzzle:1, fizzle:1,
                       gear:1, swing:1, chain:1, ring:1 };
function fxCap(){
  const q = (typeof Settings !== 'undefined' && Settings.get('vfx')) || 1;
  return Math.max(18, Math.round(FX_CAP * q));
}
function fx(o){
  o.t = 0;
  if (S.fx.length >= fxCap()){
    for (let i = 0; i < S.fx.length; i++){
      if (FX_EVICTABLE[S.fx[i].kind]){ S.fx.splice(i, 1); break; }
    }
  }
  S.fx.push(o);
  return o;
...
function skillStats(sk){
  ...
  switch (b.kind){
    ...
    case 'aoe':
      ...
      break;
    case 'aura':
      st.radius = b.radius * m.aoe * m.range;
      st.tickRate = b.tickRate / Math.max(0.35, m.cd);   // faster ticks, not shorter cd
      break;
    case 'pulse':
      st.radius = b.radius * m.aoe;
      break;
    case 'sentry':
      st.range = b.range * m.range;
      st.life = b.life;
      st.shotCd = b.shotCd * m.cd;
      break;
    case 'chain':
...
function applyElement(e, elem, ang, knock){
  ...
  switch (elem){
    case 'EMBER':
      ...
      e.burnT = 3 + M.burnDur;
      // Remember who lit it. Burn is a large share of an Ember build and used
      // to be attributed to nobody at all: tickDamage bypasses hitEnemy.
      e.burnSrc = S.src;
      break;
    case 'FROST':
...
/* ---------------------------------------------------------------------------
   SYSTEMS — telemetry (v12)

   What the player actually did, as opposed to what the game offered them. Two
   uses, and the second is the reason it is worth the bookkeeping:

     1. The run report and the results screen can say which skill did the work.
     2. The DRAFT can stop guessing. Every slot-targeted upgrade in this game
        picked its target slot uniformly at random, so LONG REACH landed on the
        skill you never press as often as the one you live on. With per-skill
        damage and cast counts, an upgrade can be offered where it will be felt.

   Attribution is by SLOT, not by shape or element, because a slot is what the
   player presses and what an upgrade targets. `S.src` is the slot currently
   resolving; it is set around a cast and read by hitEnemy, which is the single
   funnel every point of damage already passes through.

   Everything here is descriptive. Nothing in this block changes a number in the
   simulation — the draft reads it, and that is the only consumer with teeth.
--------------------------------------------------------------------------- */
const Telemetry = {
  fresh(){
    const per = [];
    for (let i = 0; i < 4; i++)
      per.push({ casts: 0, damage: 0, kills: 0, hits: 0, shape: null, element: null });
    return {
      per,
      ...
      rangeT: { close: 0, mid: 0, far: 0, none: 0 },
      rerolls: 0,
    };
  },
  slot(i){
    const t = S.tel && S.tel.per[i];
    if (!t) return null;
    const sk = S.slots[i];
    if (sk){ t.shape = sk.shape; t.element = sk.element; }
    return t;
  },
  cast(i){
    if (!S.tel) return;
    if (i === 'basic'){ S.tel.basic.casts++; return; }
    const t = this.slot(i);
    if (t) t.casts++;
  },
  damage(amount, killed, forceSrc){
    if (!S.tel) return;
    ...
    if (!t) return;
    t.damage += amount;
    if (t.hits !== undefined) t.hits++;
    if (killed) t.kills++;
  },
  /* Where the fight is being had. Called once per step from updatePlayer. */
  range(dt){
    if (!S.tel) return;
    const P = S.player, R = TUNING.close.range;
    let best = Infinity;
    Hash.query(P.x, P.y, 900, _q);
    for (const e of _q){
      if (e.dead || e.state === 'climb') continue;
      const d = dist2(P.x, P.y, e.x, e.y);
      if (d < best) best = d;
    }
    const b = S.tel.rangeT;
    if (best === Infinity) b.none += dt;
    else {
      const d = Math.sqrt(best);
      if (d <= R) b.close += dt;
      else if (d <= R * 2.6) b.mid += dt;
      else b.far += dt;
    }
  },
  /* Share of the run's damage a slot is responsible for, 0..1. The draft's
     weighting reads this; with no data yet it returns an even split so the
     first upgrade of a run is not biased by noise. */
  share(i){
    if (!S.tel) return 0.25;
    let total = 0;
    for (const t of S.tel.per) total += t.damage;
    if (total < 200) return 0.25;
    return (S.tel.per[i].damage || 0) / total;
  },
  ...
  use(i){
    if (!S.tel) return 1;
    let cTotal = 0;
    for (const t of S.tel.per) cTotal += t.casts;
    const cShare = cTotal < 12 ? 0.25 : (S.tel.per[i].casts || 0) / cTotal;
    return 0.5 * this.share(i) + 0.5 * cShare;
  },
};

/* Pick a slot to upgrade, weighted by how much the player actually uses it.
   Never absolute: the least-used slot keeps a real chance, because an upgrade
   is also how a neglected skill becomes worth pressing. Roughly 3:1 in favour
   of a skill carrying the run over one that has done nothing. */
function pickSlotByUse(cands){
  if (!cands || !cands.length) return undefined;
  if (cands.length === 1) return cands[0];
  let total = 0;
  const w = cands.map(i => { const v = 0.25 + Telemetry.use(i) * 2.25; total += v; return v; });
...
function healPlayer(amt, src){
  ...
  /* Two budgets, both refilling, both per second. `steal` is the tighter one
     because it scales with a damage stat; `healBudget` is the ceiling on
     everything — vent, salvage, field dressing, cards.

     Per-second budgets rather than stack caps, deliberately: stacking cards is
     supposed to be how you get stronger. What is not allowed is out-healing the
     deck forever, which is what the first v11 run did with four cards that each
     looked reasonable alone. A cap on the SUM is the only version of this that
     does not need re-tuning every time a healing card is added. */
  if (src === 'steal'){
    amt = Math.min(amt, P.stealBudget);
    P.stealBudget -= amt;
  }
  if (src !== 'wave'){                 // the between-wave top-up is not in-fight
    amt = Math.min(amt, P.healBudget);
    P.healBudget -= amt;
  }
  if (amt <= 0) return 0;
  const before = P.hp;
...
function ventNow(){
  ...
  if (S.tel){ S.tel.vents++; S.tel.healed += healed; }
...
function killEnemy(e, ang){
  ...
  if (V11){
    ...
    if (dist2(S.player.x, S.player.y, e.x, e.y) < R * R && S.player.hp > 0){
      ...
      gainPressure(9);
      if (chance(TUNING.close.scrapChance)){
        dropSalvage(e.x, e.y, TUNING.close.scrapHeal);
        if (S.tel) S.tel.salvage++;
      }
    }
  }
  if (M.scrapChance > 0 && chance(M.scrapChance)){
    S.pickups.push({ x:e.x, y:e.y, vx:rnd(-60,60), vy:rnd(-90,-30), t:0, life:14, heal:12, bob:rnd(TAU) });
  }
...
--------------------------------------------------------------------------- */
/* Passives run themselves. Each one still routes damage through the same
   damageArea/hitEnemy path as an active, so elements, crits, the hulk and every
   upgrade card apply exactly as they would to a cast. */
function updatePassive(idx, sk, dt){
  const P = S.player, st = skillStats(sk), E = ELEMENTS[sk.element];
  if (P.hp <= 0) return;
  /* A passive has no press, so its damage is all it can be judged on. RESTORED
     at every exit, which the first version did not do — and that single missing
     line is why a playtest report credited a Sentry with 76% of a run's damage.
     With `S.src` left pointing at the last passive that ticked, everything
     resolving later in the frame — DETONATOR bursts, OVERKILL crits, sentry
     shots, field ticks — was billed to it. */
  const _prevSrc = S.src;
  S.src = idx;
  const done = (v) => { S.src = _prevSrc; return v; };

  if (st.kind === 'aura'){
    sk.tick = (sk.tick || 0) - dt;
    if (sk.tick > 0) return done();
    sk.tick = 1 / Math.max(0.2, st.tickRate);
    const n = damageArea(P.x, P.y, st.radius, st.dmg, sk.element,
                         { silent: true, noCrit: false, from: P });
    if (n) pSparks(P.x + rnd(-st.radius*0.6, st.radius*0.6),
                   P.y + rnd(-st.radius*0.4, st.radius*0.4), 1, E.glow, 60);
    return done();
  }

  if (st.kind === 'pulse'){
    if (sk.cdLeft > 0) return done();
    sk.cdLeft = st.cd;
    sk.casts++;
    damageArea(P.x, P.y, st.radius, st.dmg, sk.element,
               { ang: P.facing, knock: st.knock, from: P });
    fx({ kind:'aoe', x:P.x, y:P.y, r: st.radius, life: 0.34,
         col: E.color, glow: E.glow });
    addTrauma(0.14);
    SFX.fire[sk.element]({ x: P.x, y: P.y });
    return done();
  }
  ...
  if (st.kind === 'sentry'){
    ...
    sk.cdLeft = st.cd;
    sk.casts++;
    S.sentries.push({ x: P.x, y: P.y, t: st.life, cd: 0, ang: -Math.PI/2,
                      element: sk.element, dmg: st.dmg, range: st.range,
                      shotCd: st.shotCd, knock: st.knock, fire: 0,
                      // whose sentry this is, so its shots are billed to the
                      // slot that planted it rather than to whatever was
                      // resolving when it happened to fire
                      src: idx });
    fx({ kind:'aoe', x:P.x, y:P.y, r: 60, life: 0.28, col: E.color, glow: E.glow });
    SFX.ready();
    return done();
  }
  S.src = _prevSrc;
...
/* A planted sentry picks its own target and fires until it expires. */
function updateSentries(dt){
  const prevSrc = S.src;
  for (let i = S.sentries.length - 1; i >= 0; i--){
    const s = S.sentries[i];
    S.src = (s.src === undefined) ? null : s.src;
    s.t -= dt; s.cd -= dt; s.fire = Math.max(0, s.fire - dt);
    if (s.t <= 0){
      pSmoke(s.x, s.y, 4, 'rgba(200,195,190,0.5)', 26);
      S.sentries.splice(i, 1);
      continue;
    }
    let best = null, bd = s.range * s.range;
    Hash.query(s.x, s.y, s.range + 40, _q);
    for (const e of _q){
      if (e.dead || e.state === 'climb') continue;
    ...
    if (!best) continue;
    s.ang = Math.atan2(best.y - s.y, best.x - s.x);
    if (s.cd > 0) continue;
    s.cd = s.shotCd; s.fire = 0.12;
    hitEnemy(best, s.dmg, { ang: s.ang, knock: s.knock, elem: s.element, silent: true });
    applyElement(best, s.element, s.ang, s.knock);
    hulkSplash({ x: best.x, y: best.y }, s.dmg);
    const E = ELEMENTS[s.element];
    pSparks(s.x + Math.cos(s.ang)*18, s.y + Math.sin(s.ang)*18, 3, E.glow, 190, s.ang, 0.4);
    fx({ kind:'muzzle', x: s.x + Math.cos(s.ang)*18, y: s.y + Math.sin(s.ang)*18,
         a: s.ang, life: 0.10, col: E.color, glow: E.glow });
    SFX.hit({ x: s.x, y: s.y });
  }
  S.src = prevSrc;
}
...
function castSlot(idx, opt){
  ...
  sk.casts++;
  // Attribution for the run: which slot is resolving right now. Restored at the
  // end of the cast rather than left set, so a bolt landing three frames later
  // is not credited to whatever fired most recently.
  const prevSrc = S.src;
  S.src = (typeof idx === 'number') ? idx : (sk === S.basic ? 'basic' : null);
  Telemetry.cast(S.src === null ? 'basic' : S.src);
...
function updatePlayer(dt){
  ...
  if (V11){
    ...
    P.healBudget = Math.min(C.healCapPerSec, P.healBudget + C.healCapPerSec * dt);
    P.crowd = crowdCount();
    Telemetry.range(dt);
    ...
    if (S.mods.dressing > 0 && P.pressure >= 50 && P.hp < P.maxHp * 0.60)
      healPlayer(S.mods.dressing * dt, 'regen');
  }
  ...
  for (let i = 0; i < 4; i++){
    ...
    if (SHAPES[sk.shape].passive){ updatePassive(i, sk, dt); continue; }
...
function spawnEnemy(type, forced, lane){
  ...
  const e = {
    ...
    burnSrc: null,   // which slot set this one alight, for attribution
...
function tickDamage(e, dmg, col, src){
  if (e.dead) return;
  e.hp -= dmg;
  S.stats.damage += dmg;
  /* Damage over time counts toward the run's attribution too, billed to whoever
     applied the status rather than to whatever is resolving now — and the
     source stays set across the KILL, because a kill triggers DETONATOR and the
     burst it causes belongs to the same slot. Without that, every kill landed
     by a burn tick spent its explosion on nobody's account, which was a quarter
     of the run's damage going unattributed. */
  const from = (src === undefined) ? e.burnSrc : src;
  const prev = S.src;
  S.src = (from === undefined) ? null : from;
  Telemetry.damage(dmg, e.hp <= 0, S.src);
...
function updatePickups(dt){
  ...
  for (let i = S.pickups.length - 1; i >= 0; i--){
    ...
    if (d < 26 && P.hp > 0){
      // through healPlayer, so salvage counts against the same ceiling as
      // everything else. Walking over one while the budget is spent still
      // consumes it — supply is not the same thing as throughput.
      const got = healPlayer(p.heal, 'pickup');
      dmgNumber(P.x, P.y - 30, '+' + Math.round(got || p.heal), '#8CE07A', 20);
...
function waveComplete(){
  ...
  healPlayer(TUNING.player.regenPerWave, 'wave');
  S.bolts.length = 0;
...
function rollCards(n){
  ...
  for (let k = 0; k < n; k++){
    ...
    // Rng, not Math.random: card rolls are gameplay randomness and the whole
    // point of a seed is that a replay deals the same hand. This was the one
    // call in the game still reaching for the unseeded stream.
...
/* Reroll (v12).
   ...
   Three cards from a pool of forty is a hand, not a decision, and a draft that
   offers a Frost upgrade to a captain running three Arc skills is asking the
   player to take something they do not want. The adaptive slot weighting above
   makes that rarer; a reroll makes it recoverable.

   Two per run, not per draft: a per-draft reroll is free and therefore not a
   decision, while a scarce pool makes "is this hand bad enough to spend on"
   a real question. Refused once a card has been chosen, and the reroll itself
   draws from the seeded stream, so a replayed seed that rerolls at the same
   moment sees the same second hand. */
function rerollDraft(){
  const d = S.draft;
  if (!d || d.chosen >= 0 || S.rerolls <= 0) return false;
  S.rerolls--;
  if (S.tel) S.tel.rerolls++;
  d.cards = (d.kind === 'upgrade')
    ? rollCards(3)
    : rollSkillCards(d.slot, 3, d.kind === 'opening');
  for (const c of d.cards) c.preview = previewCard(c);
  d.hover = -1; d.lastHover = -1;
  d.t = Math.min(d.t, 0.22);              // deal them in again
  SFX.cardDeal();
  return true;
}
