...
const TOOLS := [
	...
	{"id": "pool", "kind": "make", "script": "tools/pool_shot.gd",
		...
		"why": "a hazard drawn smaller than it burns is invisible to every other tool in this list, and this one burned you from 70% outside its own picture until SG-160 — the shape of question it answers is 'does the picture agree with the hitbox', which no screenshot can answer alone"},
