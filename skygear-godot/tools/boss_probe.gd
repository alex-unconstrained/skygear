...
## THE SEED LIST, WIDENED FROM SIX TO THIRTY-TWO (board SG-190), because with
## the clock fixed this rig is DETERMINISTIC and the six were never the sample —
## they were the whole population.
##
## Before SG-190 the `reps` argument bought real independent runs, because the
...
func _run() -> void:
	...
	## AND THE REFUSAL, IN WORDS, UNDER THE RESULT (SG-128's rule, SG-190's
	...
	if reps > 1 and duplicated > 0:
		...
		print("  rep, so those reps are copies rather than runs. THE HONEST n IS %d — the"
			% per_seed.size())
		print("  number of distinct SEEDS. Buy n with seeds, not with reps (board SG-190).")
