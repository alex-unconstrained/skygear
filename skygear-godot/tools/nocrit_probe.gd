extends SceneTree
## THE BEFORE AND AFTER FOR SG-148, on the wave-12 segment.
##
## A COPY of `tools/critx_probe.gd`, which is itself a copy of
## `tools/boss_probe.gd`, and a copy for the same reason SG-147 gave: the shared
## rig must not be modified to answer one row's question. What differs is the
## FORCED BUILD and the ARM.
##
## THE ARM. `game.secondary_can_crit` is the SG-148 gate. FALSE is the shipped
## rule and the browser's; TRUE is the build this port shipped for months. Both
## arms are the SAME BINARY and the same seeds, so nothing about the comparison
## depends on two worktrees agreeing — which is what `bal_drift.gd` exists to
## worry about. Run one arm per process and pool the `SAMPLES` lines.
##
##   godot --path . --headless --script tools/nocrit_probe.gd -- 6 0 10 SALT on
##                                                                        ^ arm
##   arm `off` = secondary sources may not crit (SHIPPED, the authored rule)
##   arm `on`  = secondary sources crit (the pre-SG-148 port)
##
## WHY `crit_chance` IS FORCED, AND FORCED IDENTICALLY IN BOTH ARMS. The six
## sources SG-148 touches diverge only on a frame where a crit is rolled, and
## `tools/bot.gd:153-169` gives every non-weapon card the same rank, so she has
## NO OPINION about crit cards at all. An unforced probe measures a population
## that mostly does not carry the thing being changed and returns a null that
## says nothing — the mistake SG-147 wrote down. 0.75 is not an arbitrary
## ceiling: `scripts/cards.gd:750` refuses the crit card above 0.75, so it is
## the most a drafted captain can reach.
##
## SO READ THE RESULT AS: what the authored rule costs A CRIT-BUILT CAPTAIN,
## which is the only population it can cost anything. A captain at the base 0.12
## sees a proportionally smaller share of the same effect.
##
...
## The ceiling a drafted captain can actually reach: `cards.gd:750` refuses
## the crit card once `crit_chance` is at or above 0.75. Read off the same
## bound the card reads rather than typed as a taste, so a tuning pass to
## that gate moves this probe with it.
const FORCED_CRIT_CHANCE := 0.75
...
func _run() -> void:
	...
	## Anything but the literal `on` is the shipped rule, so a typo cannot
	## silently produce a second copy of the pre-SG-148 arm.
	var arm_can_crit: bool = args.size() > 4 and str(args[4]) == "on"
	## THE SIXTH ARGUMENT IS THE FORCED CRIT CHANCE, and a NEGATIVE value means
	## DO NOT FORCE — draft whatever the bot drafts. Both are worth measuring and
	## they answer different questions: forced is "what does the rule cost a
	## crit-built captain", unforced is "what does it cost the population that
	## actually plays". Reporting only the forced pair would overstate the change
	## for everyone who never drafts crit; reporting only the unforced pair is
	## the null SG-147 warned about. So the probe does both and says which.
	var forced: float = float(args[5]) if args.size() > 5 else FORCED_CRIT_CHANCE
	var bw := boss_wave()
	if bw < 0:
	...
	print("  NOCRIT PROBE (SG-148) · crit_chance %s"
		% ("FORCED to %.2f in every run" % forced if forced >= 0.0
			else "UNFORCED — whatever the bot drafts"))
	print("  ARM: secondary_can_crit=%s — %s"
		% [arm_can_crit, "the pre-SG-148 port, secondary sources CRIT"
			if arm_can_crit else "the authored rule, secondary sources MAY NOT crit"])
	...
	for rep in reps:
		for i in mini(count, SEEDS.size()):
			var r := await _one(salt + SEEDS[i], heat, bw, arm_can_crit, forced)
			rows.append(r)
			print("  %-10s%-5s %s"
...
func _one(seed_text: String, heat: int, bw: int, arm_can_crit: bool, forced: float) -> Dictionary:
	...
	## FORCED BUILD — the whole reason this file is not `boss_probe.gd`.
	##
	## `bot.rank_of` returns the SAME rank for every non-weapon card, so the
	## tie-break takes whichever upgrade was offered first; she has no opinion
	## about CRIT at all. An unforced probe therefore measures a population that
	## mostly does not carry the thing SG-148 changes, and would return a null
	## that says nothing about the build. Forced identically in BOTH arms, so
	## the rig cancels and only the gate differs.
	if forced >= 0.0:
		game.mods.crit_chance = forced
	## THE ARM ITSELF, and the only line that differs between the two.
	game.secondary_can_crit = arm_can_crit
