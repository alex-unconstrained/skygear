...
func _run() -> void:
	...
	## INTO A PHYSICS FRAME BEFORE ANYTHING IS STEPPED (board SG-187).
...
func _one(seed_text: String, heat: int, bw: int) -> Dictionary:
	...
	## about CRIT EXPLOSION at all. And `critx` is a RARE card. An unforced probe
	## therefore measures a population that mostly does not carry the thing
	## SG-147 caps, and would return a null that says nothing about the build.
	## Forced identically in BOTH arms of the A/B, so the rig cancels.
