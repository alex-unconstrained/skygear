...
Last updated 2026-08-04. **Read this first, then `docs/BOARD.md` â€” the work
...
is retired â€” *"we build the Godot version to be better than the web one ever
...
argument and a measurement. The browser is still a place to LOOK â€” it is where
SG-147's real crash fix came from â€” but a diff against it is a question, never
...
a difficulty ladder, a sky, a cutscene system with all five cues filled â€”
a run-opening reveal, event-wave flourishes, the Colossus arrival, and victory
and defeat shots â€” and, since 2026-08-02, **the ship's own progression**: six
...
deck once at run start and never mid-run (the owner's rule, harness-pinned â€”
board SG-56). **1128 harness checks**; the text audit covers 25 screens at
4 widths and **is clean as of 2026-08-02, for the first time in a while** â€” the
...
WAVE IT BELONGS TO â€” both from the owner's first full twelve-wave run, Heat 2,
...
than the captain. He was not unable â€” he was at **32% of the rate**, which at
23â€“31 s of life is the same thing: **8 swings and 208 HP off a 760 cannon in 60
...
everything standing in it â€” captain, cannons, crew, Boiler, one circle, no
...
Heat 2 â€” +114% and +112%**, which is 62â€“68% of the chase build's rate while
keeping the telegraphed, escapable beat that earned the lane walk. **And the
Boiler statistic COLOSSUS-DESIGN Â§2 has been
...
comment saying every shape must be able to bite the hulk â€” and the BASIC ATTACK,
...
the answer is yes and **`get_process_delta_time()` â€” the idle frame's WALL-CLOCK
duration â€” if it is no.** A `_run` reached by `call_deferred` from
...
â€” written in `balance.gd`, `boss_probe.gd`, `melee_probe.gd` and `critx_probe.gd`,
correct, and load-bearing â€” was read by NOTHING.** Measured in a six-line probe:
...
header blames on the physics server, and that header was wrong** â€” the proof is
...
that 120 is ~4.5x too narrow â€” SG-128's exact sin in a new hat. Buy n with
...
is a question per row, and re-running is cheap now** â€” start with anything that
turned on a boarder ARRIVING somewhere. One `bot Â·` check pins the rule for the
...
swing `enemy.state_time` â€” a COUNTDOWN â€” as the window to fit the clip to, and
`want()` recomputed `speed_scale` from it every frame, so as `state_time` ran
0.90 â†’ 0 the rate climbed with it. Because `swinging` holds the rig in `swing`
for the windup AND the recovery, **the ramp ran twice per attack**, and `want`'s
`elif not anim.is_playing()` arm â€” there to restart a CYCLE the blend dropped â€”
...
and at the old window those played at 1.41x to 3.93x â€” a beat that changed by
...
its follow-through instead of being dealt again. **Worst rate 4.00x â†’ 0.98x,
plays per attack 2.50 â†’ 0.13, the simulation untouched, the captain and the
Boilerwright byte-identical.** The crew are NOT fixed and are filed rather than
folded in â€” their swing is still fitted to a countdown and nothing has measured
...
tested arrival with `distance <= reach` â€” a test that asks how far away his GOAL
...
so no arrangement of boarders can empty a lane. **The roaming is free** â€” crew
damage share 6.02% â†’ 6.30%, taken 346.0 â†’ 358.5, held 80.0% â†’ 80.4%, none of it
...
one another â€” invisible while they mill through a swing cycle, obvious the day
they stand still. Fanning them out Â±120 cost **crew damage âˆ’24% and damage taken
...
REFUSED and `POST_SPREAD` is deleted rather than set to zero â€” a knob whose only
...
`ResourceLoader`, registered in `weapons.json` â€” and on nobody. **The gap was not
...
the first failure mode in its largest coat â€” not a field with no reader, **a
whole asset class with no reader** â€” and every green signal around it stayed
...
it so the 1.8 m-frame scaling exists once: `weapon Â· a weapon is scaled by the
...
**The deliverable is the live check** â€” `weapon Â· a live deck puts the pike, the
axe and the wrench in the hands they were made for â€” the SG-170 regression` reads
...
**AND FOUR THINGS THAT WERE ALL THE SAME BUG â€” A PICTURE THAT DID NOT AGREE WITH
...
applied here â€” *a fill you can see through and an edge you cannot miss*. Now the
...
Colossus's boundary was **three times softer than a gremlin's** (`telegraph Â· and
...
recovery window â€” 200 ms at the Colossus, 130 at the furnace knight â€” so the
...
`radius * 2.2` with per-site radii of 46, 62 and `62 + 22Â·residue`, and the
hidden 2D `_draw` had a FOURTH. The Sear trail's ring drew at about 41 against a
burn of 78 â€” **an 88% gap**, and the band of deck between them looked clear and
...
decal is sized through `ring_span_for()` rather than by a multiplier â€” because
...
it walks the captain to 48Ã—20 spots and runs one real tick at each. Furthest
...
now** â€” the owner picked it off the A/B pairs (*"Cutlass clamped looks better"*),
...
survives. The agent went looking for a case where staying shiny wins â€” it chose
brass and two blades precisely because that is where such a case would live â€” and
...
under** â€” literally shinier than everything standing on them, which is most of
why the owner called them placeholder. They are retinted, roughened and under the
ceiling, and the restatement in `view3d.gd` is held to its source by `deck Â· the
...
2026-08-04) â€” the switches default OFF and nothing is decided.** His own rail
...
"the louder of the two", but at the STEM pose â€” the one his build-60 screenshot
is taken from â€” the capping is off both sides of the frame and the flat olive bar
...
the deck THROUGH it**, so DECK-IDENTITY Â§6's job is done better rather than
...
at `lift - 8`, the SAME BAND, so there is no "under" to put a second rail in â€” it
...
sweep is on the sheet and **it is the pitch, not the height, that decides** â€” div
...
2026-08-04).** `audio Â· volume survives the session` wrote 0.42 into the
player's REAL `user://settings.cfg` and read it straight back â€” and the itch
...
iterations lost **53â€“79** of 200 on `user://settings.cfg` and **0 of 200** on
...
machine â€” and this one was also quietly writing the owner's volume down to 0.42
...
and overwrote `user://keys.cfg` â€” **SG-182, FIXED 2026-08-04, and the sweep
...
`_new_game` keeps the workshop in memory, but `_fittings()` Â§4 wrote a fixture
save to the real file twice and restored it from a string afterwards â€” his
...
as the last thing the run does** â€” `editor Â· the harness never touches the
player's own saved layout`, `editor Â· and it never touches the player's own
settings either`, `editor Â· and it never wipes the player's own run history â€” the
SG-182 regression`, `editor Â· nor the keys he rebound`, and `editor Â· nor a byte
of what he has earned` â€” and the standard of proof is the tree rather than the
...
can jump to any heat, that would be better for my testing"*).** SETTINGS â†’
**OPEN ALL HEATS** â†’ ON, then pick any rung on the title. It persists in
...
has been earned. **The gate is not deleted and the ladder still climbs** â€” two
...
ambiguous: a run played above the rung you have earned banks NOTHING** â€” no
...
â€” a Heat 0 run with the bypass on banks in full, which is also what makes the
...
are `heat Â· with the bypass off the ladder still climbs one rung at a time â€” the
SG-160 regression` and `heat Â· a run above the rung you have earned banks
...
`best_heat` 0 â€” a bypassed clear â€” and got its "no sigil" from that rather than
from Â§5's rule.
**THE TWO MELEE HEAVIES WERE MEAT SHIELDS AND ARE NOT ANY MORE â€” AND THE HONEST
HALF IS THAT MAKING THE COLOSSUS 21% MORE DANGEROUS MADE THE RUN 12.7 POINTS
EASIER (SG-165 + SG-166, 2026-08-03).** The furnace knight's `hp` is **360** â€” the
...
then n=47: he **lives 2.70 s â†’ 5.79 s**, resolves **0.83 â†’ 2.49 swings**, and
deals **26.6 â†’ 84.6 damage â€” 27% â†’ 85% of a captain's whole pool**, on
...
â€” *"have him ignore the player and keep heading towards the middle"* â€” and his
...
chain, so **0.00 â†’ 150.21**, and the zero is a harness check now, asserted from
both sides in one fixture. **THE FINDING THAT GENERALISES IS ABOUT VARIANCE.**
Four arms of ~~n=480 runs each~~ â€” **and BOTH halves of that sample claim were
...
escapable circle has no tail at all â€” sd 70.3 â†’ 36.9.** The shipped arm deals
**+21.2%** (95% CI of the difference +18.3..+34.2) and holds **78.5% [74.6â€“82.0]
against 65.8% [61.5â€“69.9]**: both sentences are true at once, and every future
telegraph design should expect it â€” **legibility does not only convert damage
into fairness, it deletes the outlier runs that were doing the killing.** Putting
the hold-rate back is one line and `docs/COLOSSUS-DESIGN.md` Â§1b has three
measured points to choose from. **What is still NOT true is the Boiler clock**:
with the gate on, Boiler HP lost in wave 12 went 11.08 â†’ **6.00**, DOWN, because
he now spends 43% of his life planted â€” ~~Â§2's amended statistic has failed three
...
statistic is retired at `docs/COLOSSUS-DESIGN.md` Â§2 and the lever is his
...
malformed row silenced THREE whole reports â€” the LETHAL arm, the only thing that
...
EVER READ IT â€” the parameter was spelled `_source` while `enemy.gd` passed the
...
SEGMENT rather than the whole run â€” because a whole-run mean is the wrong
instrument for one wave in twelve, which is what SG-119 paid for.
**COLOSSUS-DESIGN Â§1 said his swing "geometrically cannot land" on a moving
...
health did NOT buy a longer boring fight** â€” wave 12 got 24% longer while damage
taken in it rose 189%, so danger per second rose 157% â€” and the honest cost is
...
failed Â§2's own pre-committed statistic (Boiler HP lost 9.5 â†’ 11.3 against a
required +40) and it takes his damage to the captain to a structural zero. Â§1a of
...
it, and a check that can only read a shipped default is vacuous â€” deleting the
...
unconditionally â€” a **360Â° circle at 163** â€” while `view3d.gd` drew him a **120Â°
...
re-deriving. **His swing lands at exactly the same distance it always did** â€”
...
bug**: `telegraph Â· a boarder that draws no wedge keeps its circle` passed every
run for weeks under a comment claiming he "telegraphs with a phase ring rather
than a fan" â€” a renderer that did not exist. That check is INVERTED, in place,
...
over a full circle and the share that lands must equal the arc that is drawn â€”
...
`set_seed_text` promised in its own comment that a seed reproduces a run â€” which
...
**11.4 dps against its authored 12.0 in every build that ever shipped** â€” which
...
fixed here** â€” the steam taps and the player's aura and pulse passives, filed as
...
*everything* â€” a boarder's 26-damage swing included â€” while the pool itself
...
threaded `damage_player` â†’ `take_damage`, default TRUE so every discrete hit is
byte-for-byte unchanged, FALSE from the tick â€” and the sweep for other periodic
...
Five `hazard Â·` checks, the pre-committed one being `hazard Â· a fire tick never
buys immunity to a swing`.

**And the second half is the fifth failure mode again, one level deeper than it
has been found before.** `tools/balance.gd` â€” the source of every simulated-run
verdict in these docs â€” carried a comment saying its bot *"keeps moving"* beside
...
(but does not route around one it is clear of â€” a bot that dodged fire perfectly
...
frames (`set_process(false)` on the game never reached her â€” she is her own
...
different game**: Heat 0, ~~n=120~~ â€” damage taken **34.6 â†’ 241**, runs held
**38% â†’ 92%**, close-range time **3â€“5% â†’ ~22%**. **The SAMPLE was corrected
2026-08-04 (SG-190): that "n=120" is SIX SEEDS at twenty reps, so it is n=6 on
the axis that carries the variance you care about** â€” the reps differed only by
...
held-rate observation â€” the one waiting on the owner's threshold â€” has flipped
...
otherwise.** The residual is below the scene tree â€” `move_and_slide()` queries a
physics space the server syncs on its own tick â€” so the header states plainly
...
TWICE â€” 2026-08-04.** Both halves of that paragraph are now known to be wrong,
...
`move_and_slide()` takes `get_process_delta_time()` â€” the idle frame's
WALL-CLOCK duration â€” whenever it is called outside a physics frame, which every
...
(âˆ’10%) was reported to the owner as *below the rig's resolution* rather than as
a finding â€” conservative, and by SG-128's later arithmetic it had actually
cleared its bar (t=2.43, p=0.015) â€” and the fix's real evidence is the direct
probe either way. Three `bot Â·` checks; the one that matters is `bot Â· the
bot actually moves the captain â€” the SG-118 regression`.
...
model is not the problem**: its seating â€” the exact thing SG-157 refused the
first stern for â€” is fixed by construction, and seated correctly the A/B pair
...
identically, because this camera never sees the outside of the hull** â€” it sits
...
longest axis, so "both are 1.9 wide" is not a statement about the ship â€” the
stern's mirror plane says 189 of it runs FORE-AND-AFT and only 90 goes across.
Four `edge Â·` checks; the sheets are `.shots/owner-review/2-bow-stern-redo/`.
...
corner post, ingested at budget with **no decimation at all** â€” he generates at
...
satisfies all three of the constraints that were in tension â€” a stair under the
...
21.72% at the stem and 0.00% from mid-deck at the shipped zoom** â€” the second
...
it back is to stand in the rectangle. Four `edge Â·` checks; the sheets are
...
per-MODEL-KEY table â€” omni or spot, colour, strength, reach, falloff, offset in
ground units, a spot's cone and aim, an optional pulse or flicker â€” read by
...
own lamps, SAVE. Not the clipboard â€” the renderer got the reader first. **Model
...
energy cannot pass 7.5 â€” which is the 7.39 the deck already carried, so the
...
Boiler, and the furnace knight â€” whose chest light is SG-86's named candidate
and measurably lifts him (30.4 â†’ 33.4 mean luminance, 10 â†’ 276 hot pixels).
...
that are genuinely RENDERER constants â€” the bloom over emissives, a scale on the
impact particles' bodies, and how long one of them lives â€” read by
...
run. **Two of the three were moving nothing at all before this** â€” GLOW wrote a
...
wearing the fix's clothes; `view Â· every dial in the fx table is read by the
renderer` and `view Â· and the renderer on screen is built out of those dials, not
...
purpose.** The railing won its second roll â€” v1 arrived standing on a solid
...
top face, which is a texture either way â€” the rope coil's standing verdict, one
...
for EVERY key in its table, including the RIGGED figures â€” so running it to wrap
...
every control under it was `SkyGearUI.button` â€” **two shape calls**, a flat tint
...
own flicker clock); BEGIN RUN is **the door** â€” taller, teal, wearing the
...
beneath it, which is what keeps the legibility pass â€” it renders each screen
twice and samples the second â€” from measuring noise. `docs/MENU-DESIGN.md` is
the survey, the vocabulary and the ordered plan; the other screens are SG-93 and
are deliberately waiting on the owner's look. **Nine `menu Â·` checks**, the text
audit clean at all four widths (and the pass turned a detector ON â€” nothing on
...
and left its painted rung behind (`menu Â· a nudged Heat rung takes its own art
with it`). `.shots/sg91/` is the witness.

**F4 RESIZES NOW (SG-80, owner ask): a selected element has a size as well as
a position** â€” drag a grip, type into the wÃ—h readout, or hold Ctrl and use the
...
**And Ctrl+S was genuinely broken (SG-83) â€” but not where anyone would look:**
...
also says `SAVED Â·` with the real path, or shouts if a write fails.
...
particles have bodies** â€” every one of them was a flat card swung to face the
...
`burst` â€” every kill, every powder keg, the hulk coming apart â€” had nothing in
the air at all**: VFX-PLAN Â§3/Â§4 gave every other shape geometry in July and
...
effects** â€” the Pulse ring, the fire fields, the Colossus turn ring, the
Boilerwright's vent stand, and the aura edge a card WIDENS â€” because they all
...
skill's reach and dimming past it, is now the whole feature â€” the ring drew as a
flooded opaque disc because it went through the painted `rune_player.png` plate,
which is a FILLED disc, so DESIGN Â§13e's premultiplied emission map lit the whole
projection box. The captain's cape is OFF by default and `HERO_CLOAKS` is empty
(two verdicts: "looks horrible", "atrocious") â€” the cloak code stays in the build
...
the painting it replaced. All ten wired prop rows are pinned within Â±10% of intent.

**Alignment is FIXED in the game now, not filed from screenshots** (SG-42, the
owner's ask): press **F4 on the screen that is wrong** â€” any screen â€” and move
...
any of the audit's screens on a sandbox â€” edit GAMEOVER without dying, the
results without winning â€” and Esc hands the game back exactly, mid-run
included (`editor Â· and leaving the pose hands the run back exactly`).
...
photograph all 25 screens at all 4 widths as one page â€” for auditing everything
at once; fixing is F4), and `layout` (promote the F4 alignment â€” plates, items
and per-screen element offsets â€” out of `user://` and into the repo, which is
...
- **A measuring rig nobody measured â€” the newest one, and it cost the most.**
  ...
  particle clock, then â€” the largest â€” **every rigged figure's `AnimationPlayer`,
  ...
  must first report its own noise floor, and that floor must be exactly zero** â€”
  not small, zero. A floor allowed to be 3% is a floor that can hide a 3%
  feature. `SkyGear Tools.bat still` is that assertion; the five `still Â·` checks
  in the harness are what stop the next tool from skipping it.

- **A fact known in one place and contradicted in another â€” the newest, and the
  ...
  *"balance.gd's bot never moves the captain"* and reasoned correctly from it,
  ...
  LAST place it belongs â€” fix the tool's comment, and name the rows that
  ...
  **And the cheapest possible version of it, SG-188, 2026-08-04: a comment that
  ...
  comment beside the crossing even asserted the fix was already in place â€”
  *"`want` re-reads the window only on the frame the state changes"* â€” which was
...
- **A harness check that asserts the bug.** Distinct from silencing a detector:
  here the detector is loud, green, and wrong. `telegraph Â· a boarder that draws
  ...
  â€” and the one melee row missing from that list was the only row that could
...
thousand â€” 61 hours. So a small sample that comes back "no difference" is not
evidence of no effect; it is an instrument that could not have seen one, which is
the mistake SG-125 exists to catch. **The old rule this replaces â€” "the rig's
floor is 8%, disbelieve anything under it" â€” was wrong, and wrong in BOTH
...
cost, under every result. `n=6` â€” the default before SG-118 added `reps` â€” is
...
project). Leave them alone â€” they are the owner's, they are not the port, and
...
uncommitted in the same file, and `git add tests/parity_test.gd` took all of it â€”
...
know â€” it is the only thing that catches this, and it caught it. If you share a
...
file â€” **and that goes for the screenshot tools too**: `screens`/`text`/
...
- `docs/SHIP-AND-MAPS-DESIGN.md` -- maps, run diversity, ship progression,
  ...
  (SG-15); the stowage spine was built and then CUT by its own Â§7.1 kill-test
  (SG-48); Â§5's fittings + berths + berth screen were BUILT 2026-08-02 under
  the owner's between-runs reframe (SG-56). Still unbuilt: Â§6's walkable
...
- `VFX-PLAN.md` Â§6, the blade-driven weapon trail (SG-18). The two remaining
  Articles â€” The Opening Bid and The Second Hand â€” were BUILT 2026-08-01
  (SG-26), each with its trade live in the sim. Heat 3-5 were built 2026-08-01; Â§5's chromatic/radial half was
...
**Character models â€” the owner's standing rules (2026-08-01).** These exist
...
- **Clean T-pose or A-pose only.** Prompt for it, and reject a generation that
  comes back posed â€” retargeting needs a neutral rest pose to work from.
...
- **Weapons are separate models**, mounted via `BoneAttachment3D` â€” the
...
- **Capes, cloaks and loose cloth are separate too.** The captain's cape was
  ...
  on a chest mount â€” trails at a run, cracks on the dash, bitwise-still at
  rest when sway is off (`cloak Â·` checks, seven of them; `.shots/cloak/`).
...
owner ran his mesh through Mixamo and delivered the Great Sword Pack â€” his
native rig plus 51 clips â€” and the whole thing ingested as one `models.json`
entry through `tools/ingest_model.py` (the Â§13l axe-pack path, second use).
Five heavy slashes rotate as his attacks, and Tap Main plays a real kneel
(`figure Â· tapping a main plays the plant, and the kneel fits the tap window
...
2026-08-02) â€” the second rigged boarder, and the first figure in this game that
...
closes SG-22 and empties the `handoff-3d/` queue. He stands 216 ground units â€”
the SIM's own number for the archetype, a head over the captain â€” and he WALKS,
...
before it sinks (`figure Â· and the body does not start leaving until the death
has been played`). His furnace chest is the first emission map this pipeline
has ever read â€” and the honest note beside it is SG-86: on the deck he is about
...
182 MB to 9 MB (the captain deliberately excluded â€” board SG-13); the itch zip
is 94.5 MB.
