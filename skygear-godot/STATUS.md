...
board SG-56). **1037 harness checks**; the text audit covers 24 screens at
...
EASIER (SG-159 + SG-160, 2026-08-03).** The furnace knight's `hp` is **360** — the
...
(SG-161, SG-162):** `melee_probe` read `game.wave` after `queue_free()`, and ONE
...
`harness` (1037 checks), `text` (the audit), `screens` (the BATCH-evidence mode:
...
| `tests/parity_test.gd` | 1037 checks; the closest thing to a specification |
...
| | |
|---|---|
| `harness` | 1037 checks. Green before anything ships |
