...
function drawHealthPanel(){
  ...
  // One tick per 20 max HP, capped. Uncapped this is a draw call per 20 hp for
  // as long as the bar exists: the profiler set maxHp to 1e9 to take health out
  // of a measurement and got 50 MILLION stroke calls in a single frame out of
  // this one line. No real run reaches that, but nothing in the HUD should scale
  // without a ceiling, and a segment every 20hp stops being readable past ~20.
  const segs = clamp(Math.round(P.maxHp / 20), 1, 20);
