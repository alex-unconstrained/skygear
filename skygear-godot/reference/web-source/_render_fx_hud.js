...
function drawGroundFx(){
  ...
  /* Level of detail, by measurement rather than by taste. Every ground effect
     draws its shape twice: once in its element colour and once more in additive
     glow. The second pass is the expensive one and the least missed — when
     thirty of them are on the deck at once nobody is looking at the bloom on any
     single one. Above the threshold the glow pass is dropped, which took a
     saturated frame from 9,354 canvas calls to about a third of that.

     GAMEPLAY BOUNDARIES ARE NOT TOUCHED BY THIS. The shape, radius and duration
     of every effect stay exactly as they are; only the second cosmetic pass over
     the same shape goes. That rule is from the event sheet (V10-PLAN §4). */
  const lod = S.fx.length > 26;
  for (const f of S.fx){
