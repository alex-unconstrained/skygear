...
func _sync_effects() -> void:
	...
	for enemy in game.enemies():
		...
		if enemy.state == "stomp":
			## THE STOMP, DRAWN (board SG-160). The owner asked for "damage around
			...
			if flash > 0.0:
				...
				## AND IT FLASHES THE SHAPE THAT ACTUALLY LANDED (SG-160). `recover`
...
static func _decal_class(key: String) -> DecalClass:
	...
	## `ts` joined them with SG-160 — the Colossus's stomp: the held outline, the
...
func _sync_all(delta: float) -> void:
	...
	for enemy in game.enemies():
		...
		## (SG-160) — a Colossus who plants a 240-unit circle while playing his idle
