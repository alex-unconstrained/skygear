...
## THE ONE CAPTION ON THE SETTINGS SHEET (SG-155): two wrapped 12pt lines under
...
func _draw_title() -> void:
	...
	## OPEN ALL HEATS on (board SG-155), is true from a fresh save. That is the
...
func _draw_heat_ladder(wx: float, wide: float, top: float) -> float:
	## TWO CEILINGS, NOT ONE (board SG-155). `earned` is the progression rule and
	...
	for r in range(1, rungs + 1):
		...
		## OPENED, not earned (board SG-155): reachable only because the bypass is
	...
	if active > reachable:
		...
		## made rather than discovered on the results sheet (board SG-155). The
...
func _draw_settings() -> void:
	...
	## starting at 86.
	##
	## TEN since SG-155, plus the one caption OPEN ALL HEATS carries under it —
	...
	y += 40.0
	## OPEN ALL HEATS (board SG-155). The bypass lives HERE, on the settings sheet,
...
func _draw_results(title: String, tint: Color) -> void:
	...
	if not (game.banked as Dictionary).is_empty() and int(game.banked.get("scrip", 0)) > 0:
		chrome += 18.0
	## The bypass line (SG-155) takes the same 18 the payout line would have, and
	...
	##
	## A BYPASSED RUN TAKES THE SAME SLOT (board SG-155) and says the opposite, for
