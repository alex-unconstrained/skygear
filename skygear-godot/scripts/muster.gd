...
static func plan(seed_text: String, wave_number: int, heat: int,
	...
	if not changed.is_empty():
		pool = changed
	## SG-201. PINCER is a lane grammar, so a type-only substitution that happens
	## to leave an already-Pincer-shaped authored wave inside 35/35/30 is a valid
	## plan but a weak forced arm. Prefer the candidates that actually move one
	## authored source to another lane whenever the finite legal pool contains
	## one. The fallback matters: a naturally Pincer-shaped wave with no legal
	## lane move can still take a roster substitution rather than returning
	## AUTHORED. This filters before the existing single draw and consumes no RNG.
	if picked_grammar == "PINCER":
		var lane_changed: Array = []
		var authored_lanes := _source_lane_signature(authored)
		for candidate in pool:
			if _source_lane_signature(candidate) != authored_lanes:
				lane_changed.append(candidate)
		if not lane_changed.is_empty():
			pool = lane_changed
...
## Types and counts are deliberately absent: this identifies whether the plan
## moved an authored allocation, which is the behavioral half of PINCER.
static func _source_lane_signature(batches: Array) -> String:
	var unique := {}
	for batch in batches:
		for lane_value in batch.get("lanes", []):
			unique["%d|%d" % [int(batch.get("source", 0)), int(lane_value)]] = true
	var pairs: Array = unique.keys()
	pairs.sort()
	return "|".join(pairs)
