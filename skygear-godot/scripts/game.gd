...
func cast_skill(index: int, aim_at = null) -> void:
	...
	if float(mods.residue) > 0.0:
		...
		## by it — the tick has always used one flat radius (SG-160). `dps` is in
		## the same position and is deliberately LEFT, because it is a damage rate
		## and the owner decides those: board SG-161.
...
func bleed_jet(direction: Vector2) -> bool:
	...
	for i in steps:
		...
		## only the picture, which is exactly the 46-drawn/78-burned gap SG-160
		## closes. It is no longer read; `trail_dps` is left for board SG-161.
		_field({"position": at,
...
## HOW FAR A FIRE POOL BURNS — AND, SINCE SG-160, HOW BIG IT IS DRAWN.
##
...
func _update_fire_fields(delta: float) -> void:
	for i in range(fire_fields.size() - 1, -1, -1):
		...
		while float(field.tick) <= 0.0:
			...
			_damage_circle(field.position, fire_pool_radius(), 7.5, "EMBER", 0.0, false, false, secondary_can_crit)
...
func _field(d: Dictionary) -> void:
	_fx_seq += 1
	d["id"] = _fx_seq
	## A POOL'S RADIUS IS THE POOL'S, NOT THE CALLER'S (board SG-160). The three
	...
	## smaller than a residue pool is a live question and it is board SG-161 —
...
func _draw() -> void:
	...
	for field in fire_fields:
		...
		## The hidden 2D scene, and it read 78.0 as a literal too (SG-160). The rim
