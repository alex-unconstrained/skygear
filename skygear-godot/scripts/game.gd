...
var heat := 0
## OPEN ALL HEATS (board SG-155). The settings sheet's playtest bypass, mirrored
...
func _ready() -> void:
	...
	## settings visit later (board SG-155).
...
## OPEN ALL HEATS (board SG-155), on the same shape as `toggle_fullscreen` and
...
func begin_run() -> void:
	...
	## OPEN ALL HEATS on (board SG-155), Heat 5 rather than Heat 2, because the
