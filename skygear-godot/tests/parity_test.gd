...
func _fittings() -> void:
	...
	## TABLED (board SG-68): while the crate-verb family is down, a
	## verb-granting fitting's row is OFF the table by design — so the old
	## cross-link check (`fittings · a verb-granting fitting names a real verb
	## row, and the row names it back`) is rewritten as a PAIR: today the
	## verb-granting fittings are exactly the TABLED set (`tabled()` and the
	## table agree), and with the flag lifted the original cross-link must
	## hold verbatim, because the revisit inherits it.
	var tabled_right := true
	var tabled_note := ""
	for id in SkyGearFittings.FITTINGS.keys():
		var verb := str((SkyGearFittings.FITTINGS[id] as Dictionary).get("verb", ""))
		var is_tabled := SkyGearFittings.tabled(str(id))
		tabled_right = tabled_right and is_tabled == (verb != "")
		tabled_note += " %s:tabled=%s" % [id, is_tabled]
	_check("fittings", "the tabled fittings are exactly the verb-granting ones while the family is down",
		tabled_right, tabled_note)
	SkyGearGame.CRATE_VERBS_ENABLED = true
	var verb_link := true
	var link_note := ""
	for id in SkyGearFittings.FITTINGS.keys():
		var verb2 := str((SkyGearFittings.FITTINGS[id] as Dictionary).get("verb", ""))
		if verb2 == "":
			continue
		var found := false
		for spec in SkyGearDeckwork.actions():
			if str(spec.id) == verb2 and str(spec.get("fitting", "")) == str(id):
				found = true
		verb_link = verb_link and found and not SkyGearFittings.tabled(str(id))
		link_note += " %s<->%s:%s" % [id, verb2, found]
	SkyGearGame.CRATE_VERBS_ENABLED = false
	_check("fittings", "and with the flag lifted every verb-granting fitting names its real row again, and the row names it back",
		verb_link, link_note)
...
func _view() -> void:
	...
	## what `heat_available` has always meant. Implicit until SG-155 made a run
	...
	## --- OPEN ALL HEATS, the playtest bypass (board SG-155) ---------------------
	##
	...
	_check("heat", "with the bypass off the ladder still climbs one rung at a time — the SG-155 regression",
	...
	## THE PLANT'S SIM SIGNAL (SG-74). The renderer kneels him for the first
	## PLANT_WINDOW after a tap by reading `tap_cooldown` — `tap_main()` writes
	## the full cooldown on the frame the tap lands, so cooldown-minus-remaining
	## is the time since he cracked it. Pin that arithmetic on the frame of the
	## tap: elapsed must read as zero, inside the window, and the window itself
	## must be shorter than the cooldown or the kneel would never end.
	var tap_elapsed: float = float(SkyGearData.TAP.cooldown) - game.tap_cooldown
	_check("class", "tapping a main opens the plant window the renderer reads off tap_cooldown",
		game.tap_cooldown > 0.0 and tap_elapsed < SkyGearRig3D.PLANT_WINDOW
			and SkyGearRig3D.PLANT_WINDOW < float(SkyGearData.TAP.cooldown),
		"elapsed %.2fs of a %.2fs window (cooldown %.1fs)"
			% [tap_elapsed, SkyGearRig3D.PLANT_WINDOW,
				float(SkyGearData.TAP.cooldown)])
	...
	## The Boilerwright's own rig — his Meshy mesh, exported as an OBJ and
	## auto-rigged by MIXAMO on native mixamorig_* bones (SG-74), with the
	## owner's Great Sword Pack ingested onto it through the archive path:
	## 51 native clips, one consistent 57.4 degree retarget, our maps re-applied
	## from .model_originals (the captain precedent — the Mixamo FBX is
	## untextured). This retires the SG-12 construction (captain's axe clips
	## re-retargeted through a Meshy bone_map). His scene is committed, so these
	## assert the real file the way the captain's rig checks above do.
	...
	if ResourceLoader.exists(bw_scene):
		...
		if bw_built:
			## Every clip name the renderer actually ASKS his rig for must be
			## aboard: idle, the walk/run pair (walk is the run's own fallback
			## tier), the five heavy cuts plus the spin (the VARIANTS
			## rotation), the Bleed Jet's dash, the flinch, and the plant —
			## Tap Main's kneel, the clip CLASS-2 §7 called the one with no
			## off-the-shelf equivalent, delivered in the owner's own pack.
			## Deliberately NOT in this list: idle_alt/block/taunt (aboard,
			## named to the captain's vocabulary, read by nothing for EITHER
			## class today) and run_back (not in this pack — his backpedal
			## plays the forward run, recorded on SG-74's row, not hidden).
			var bw_wired := ["idle", "walk", "run", "swing", "swing2",
				"swing3", "swing4", "swing5", "spin", "dash", "hurt", "plant"]
			var missing_wired := PackedStringArray()
			for c in bw_wired:
				if not bwrig.has_clip(str(c)):
					missing_wired.append(str(c))
			_check("figure", "his native rig carries every clip the renderer is wired to ask for",
				missing_wired.is_empty(),
				"%d of %d (missing: %s)" % [bw_wired.size() - missing_wired.size(),
					bw_wired.size(), ", ".join(missing_wired)] \
					if not missing_wired.is_empty() \
					else "%d of %d, out of %d aboard" % [bw_wired.size(),
						bw_wired.size(), bwrig.anim.get_animation_list().size()])
			## THE PLANT FITS ITS WINDOW through the same clip-stretched-to-
			## window machinery every swing rides — the POST-PARITY item 6
			## contract. Asked of the rig itself, not remodelled here.
			bwrig.state = "idle"
			bwrig.want("plant", 0.0, SkyGearRig3D.PLANT_WINDOW)
			var plant_ok: bool = bwrig._clip == "plant"
			var plant_rate: float = bwrig.anim.speed_scale
			var plant_shown: float = 0.0
			if plant_ok:
				plant_shown = SkyGearRig3D.PLANT_WINDOW * plant_rate \
					/ bwrig.anim.get_animation("plant").length
			_check("figure", "tapping a main plays the plant, and the kneel fits the tap window like a swing fits its cast",
				plant_ok and plant_rate <= SkyGearRig3D.ATTACK_RATE_MAX
					and plant_rate >= SkyGearRig3D.ATTACK_RATE_MIN
					and plant_shown >= 0.9,
			...
			## HONESTY OVER the old 4000 line (SG-74): the OBJ the owner ran
			## through Mixamo carried the REFINE mesh, not the 3,093 remesh, so
			## his native skinned mesh landed at 12,476 — and a mesh cannot be
			## safely decimated AFTER skinning, which is SG-13's whole lesson
			## on the 30,634-tri captain. Pinned where it landed so it cannot
			## creep, not waved through: a re-export of the remeshed OBJ back
			## through the same Mixamo pack brings him under 4k for zero
			## credits, and the ask is on SG-74's row and in NEEDS_ALEX.
			_check("figure", "and his triangle count is pinned — heavier than the remesh (recorded), still well under the captain's SG-13",
				bwtris > 0 and bwtris <= 13000,
				"%d tris (remesh was 3093; the captain's SG-13 is 30634)" % bwtris)
...
func _menu() -> void:
	...
	## --- THE BYPASS ON THE REAL TITLE (board SG-155) ---------------------------
	##
	## Everything above this point in the SG-155 work is arithmetic on a state
