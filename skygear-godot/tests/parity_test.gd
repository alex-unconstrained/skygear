...
func _view() -> void:
	...
	## ---- THE EDGE OF THE DANGER ZONE IS A LINE (SG-159) ---------------------
	##
	...
	## ---- THE FIRE POOL'S PICTURE IS ITS BURN (SG-160) -----------------------
	##
	...
	## picture must refuse to follow it. Before SG-160 this decal read 46·2.2·0.82,
	...
	_check("hazard", "the burn radius did not move — the picture moved, not the damage",
		...
		"burn radius %.1f (78.0 before SG-160)" % SkyGearGame.FIRE_RADIUS)
	game.fire_fields.clear()
	view._process(0.05)
