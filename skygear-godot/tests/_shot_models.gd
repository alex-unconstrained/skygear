...
func _run() -> void:
	...
	if argv.size() > 0:
		tag = str(argv[0])
	DirAccess.make_dir_recursive_absolute("res://../.shots/models/%s" % tag)
	...
	## view3d.gd's lens, not a similar one: the same 2*atan(REF_HEIGHT/2 / FOCAL),
	## the same CAM_HEIGHT above the deck and CAM_NEAR back along it. A model
	## looked at through anything else is a model looked at at the wrong size.
	var cam := Camera3D.new()
	cam.fov = rad_to_deg(2.0 * atan((SkyGearView3D.REF_HEIGHT * 0.5) / SkyGearView3D.FOCAL))
	cam.position = Vector3(0.0,
		SkyGearView3D.CAM_HEIGHT * SkyGearView3D.WORLD_SCALE,
		SkyGearView3D.CAM_NEAR * SkyGearView3D.WORLD_SCALE)
	...
	for k in keys:
		...
		for _i in 4:
			await process_frame
		var shot := "res://../.shots/models/%s/%s.png" % [tag, k]
